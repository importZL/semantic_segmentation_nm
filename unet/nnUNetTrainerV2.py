...
from util.data_augmentation_moreDA import get_moreDA_augmentation
from unet.nnUNetTrainer import nnUNetTrainer
...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def initialize_network(self):
        ...
        if torch.cuda.is_available():
            self.network.cuda(int(self.opt.gpu_ids))
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        # print(data.size()) # [9, 1, 40, 56, 40]
        # print(target[0].size()) # [9, 1, 40, 56, 40]
        ...
        if self.fp16:
            with autocast():
                ...
                output_aug = self.network(aug_data)
                del data, aug_data
                l = self.loss(output, target)
                l_aug = self.loss(output_aug, aug_target)
